...
public function verifyCode(
    ...
    $inputCode = $request->request->get('verificationCode');

    if ($inputCode != $user->getNumberVerification()) {
        $this->addFlash('error', 'Invalid verification code.');
        return $this->redirectToRoute('app_confirm_email');
    }

    // Si le code est correct
    $this->addFlash('success', 'Double Factor Authenticator is on. Thanks for confirming your email.');
    $user->setIsConfirmed(1);
    $user->setNumberVerification(random_int(100000, 999999));
    $user->setToken(random_int(1000, 9999));
    $entityManager->flush();

    return $this->redirectToRoute('app_confirm_email'); // ou autre route de ton choix
}
